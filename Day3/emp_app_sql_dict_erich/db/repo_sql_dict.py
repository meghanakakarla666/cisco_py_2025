...
from .exc import EmployeeAlreadyExitError,DatabaseError,EmployeeNotFoundError
...
def create_employee(employee):
    ...
    try:
        ...
        raise EmployeeAlreadyExitError(f"employee id={employee['id']} exists already")
    except SQLAlchemyError as ex:
        session.rollback()
        logging.error("database Error occurred while creating employee:%s",ex)
        raise DatabaseError(" error occurred in creating employee")
